...
class ManagerDashboardOpenIdeas extends React.Component {
  ...
  searchThroughIdeas = event => {
    ...
    let ideasCopy = filteredIdeas.slice();

    let SearchedFilteredIdeas = ideasCopy.filter(el => {
      return (
    ...
    this.setState({
      searchText,
      filteredIdeas: SearchedFilteredIdeas
    });
  };
  ...
  render() {
      ...
      return (
        <div key={i} className="managerIdeaCard margin-bot-30">
          ...
          <div className="flexed-div spacedBetween margin-top-15">
            <div className={`progressCircles ${el.status === 'Submitted' ? 'activeStat' : ' '}`}>1</div>
            <div className={`progressCircles ${el.status === 'Validation' ? 'activeStat' : ' '}`}>2</div>
            <div className={`progressCircles ${el.status === 'Development' ? 'activeStat' : ' '}`}>
              3
            </div>
            <div className={`progressCircles ${el.status === 'Pitch' ? 'activeStat' : ' '}`}>4</div>

            <div className={`progressCircles ${el.status === 'Implementation' ? 'activeStat' : ' '}`}>
              5
            </div>
          </div>
    ...
    return (
      <div>
        ...
        <div className="main-container flexed-div spacedBetween flexed-wrap">{displayIdeas}</div>
      </div>
      // <div>
